...
data_base_dir = '~/for_matt/letango_collection/ce031713f9d0b8060d_test_case_12';
data_sub_dir = '';
iteration = 'iter_1';
vicon_object_file = 'target_poses.txt';
kViconGlobalOffset = .00;
...
plane_folder_1 = fullfile(data_path, 'plane_test_output', iteration, [desktop_program '_plane_1']);
plane_folder_2 = fullfile(data_path, 'plane_test_output', iteration, [desktop_program '_plane_2']);
...
tango_pose_file_1 = fullfile(data_path,'plane_test_output', iteration, [desktop_program '_tango_1'], 'tango_poses.txt');
tango_pose_file_2 = fullfile(data_path,'plane_test_output', iteration, [desktop_program '_tango_2'], 'tango_poses.txt');
...
%% Plot trajectories.
% tango_fig = figure();
% plot3(tg_p_i_1(:,1), tg_p_i_1(:,2), tg_p_i_1(:,3), 'r');
% axis equal;
% 
% vicon_fig = figure();
% plot3(vg_p_targ(:,1), vg_p_targ(:,2), vg_p_targ(:,3), 'b' );
% axis equal;
...
for i = 1 : size(tango_combined_timestamps,1)
   ...
   if ~isempty(h_1)
      ...
      imu_heights_1 = [imu_heights_1; h_1(1) + tg_p_i(3)];
   end
   ...
   if ~isempty(h_2)
      ...
      % Save heigth in {IMU}
      tg_p_i = tg_p_i_2(tango_idx_2, :);
      imu_heights_2 = [imu_heights_2; h_2(1) + tg_p_i(3)];
   end   
%    disp(num2str(i));
end
...
plot(tango_combined_timestamps, vicon_heights + kViconGlobalOffset, '-r');
